...
For common probability distributions, R features the following four commands:
...
...`s` will be stored as a matrix. This can be annoying, because it prevents you from using `s` for simple tasks like scalar multiplication:
...
Whereas the mathematical concept of a scalar is stored a `vector` object in R, the mathematical concept of a vector *sometimes* requires the object type `matrix`.
...
When objects contain attributes such as `names()` (`list`, `data.frame`), `rownames()` and `colnames()` (`matrix`), or `dimnames()`, we can also index by names:
