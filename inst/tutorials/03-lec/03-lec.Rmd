...
- The fairness of a coin can be expressed through a *probability parameter*, $\pi$, that governs the probability that a coin flip produces heads (H) has opposed to tails
- We start out with the believe that the coin is fair -- that is, we consider it more probable that the coin is fair ($\pi \approx 0.5$) and less probable that it systematically over-produces either heads or tails
...
- We stipulate that the data-generating process that produces $y$ is normal: $\mathbf{y} \sim \text{N}(\mu, \sigma^2)$.
...
- Let's pretend our prior belief is completely naive: 
    ...
    - Our prior belief for $\mu$ should thus be agnostic as to whether people like or dislike the AfD and sufficiently vague to allow for the possibility that we may be wrong: $\mu \sim \text{N}(\theta = 0, \sigma^2 = 10)$
