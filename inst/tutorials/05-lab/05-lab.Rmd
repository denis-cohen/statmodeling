...
We know want to apply the varying intercept, varying slope logit model to our running example of vote choice for the AfD.
...
$$\begin{split}
\eta_{ij} & = \gamma_{11} + \gamma_{21} u_j + \eta_{j1} + (\gamma_{12} + \gamma_{22} u_j + \eta_{j2}) x_i + \epsilon_i \\
\eta_{ij} & = \gamma_{11} + \gamma_{21} u_j + \eta_{j1} + \gamma_{12} x_i + \gamma_{22} u_j x_i + \eta_{j2} x_i  + \epsilon_i \\
\eta_{ij} & = \gamma_{11} + \underbrace{\gamma_{12} x_i + \gamma_{21} u_j + \gamma_{22} u_j x_i}_{\text{interaction}} + \underbrace{\epsilon_i + \eta_{j1} + \eta_{j2} x_i}_{\text{composite errors}}
\end{split}$$
...
$$\frac{1}{N} \sum_{i=1}^{N} \left\{ \text{logit}^{-1}(\beta_1 + \beta_2 (\mathtt{la\_self}_i + 1) + \gamma_1 \mathtt{bl\_immig}_{j[i]} + \gamma_2 (\mathtt{la\_self}_i + 1) \times \mathtt{bl\_immig}^{\ast}) \\ - \text{logit}^{-1}(\beta_1 + \beta_2 \mathtt{la\_self}_i  + \gamma_1 \mathtt{bl\_immig}_{j[i]} + \gamma_2 \mathtt{la\_self}_i \times \mathtt{bl\_immig}^{\ast}) \right\}$$

Note that we use a hypothetical value $\mathtt{bl\_immig}^{\ast}$ in the product term but the observed values $\mathtt{bl\_immig}_{j[i]}$ elsewhere. This may seem counterintuitive at first but it is essential to separate *effect modification* induced by the moderator $\mathtt{bl\_immig}$ from its distinct effect of on the linear predictor.
...
for (i in seq_along(bl_immig_vals)) {
    # Adjust the second column of Z
  Z0[, "bl_immig:la_self"] <- bl_immig_vals[i] * X0[, "la_self"]
  Z1[, "bl_immig:la_self"] <- bl_immig_vals[i] * X1[, "la_self"]
