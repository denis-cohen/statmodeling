...
```{r blocks, echo = FALSE, out.width = '72%', fig.align="center"}
...
```{stan, output.var = "none", ex1-data1-solution, echo = TRUE, eval = FALSE}
data {
...
```{stan, output.var = "none", ex1-pars1, eval = FALSE, echo = TRUE}
parameters {
  ... declarations ...
}
```
...
```{stan, output.var = "none", ex1-pars1-solution, eval = FALSE}
parameters {
...
```{stan, output.var = "none", ex1-tpars1, eval = FALSE, echo = TRUE}
...
```{stan, output.var = "none", ex1-tpars1-solution, eval = FALSE}
transformed parameters {
...
```{stan, output.var = "none", ex1-mod1, eval = FALSE, echo = TRUE}
model {
  // priors
  ... statements ...
...
```{stan, output.var = "none", ex1-mod1-solution, eval = FALSE}
model {
  // priors
...
```{r ex1-full, echo = TRUE, eval = FALSE}
# Save as character
lm_code <- 
"data {
  int<lower=1> N; // num. observations
...
```{r inf-sim1, exercise=TRUE, eval = FALSE}
# Set seed
set.seed(20210329)
...
```{r inf-setup, echo = TRUE, eval = FALSE}
## Setup
library(rstan)
rstan_options(auto_write = TRUE)             # avoid recompilation of models
options(mc.cores = parallel::detectCores())  # parallelize across all CPUs
...
```{r inf-sampl, echo = TRUE , eval = FALSE}
...
```{r inf-real, echo = TRUE, eval = FALSE}
